...
export function Dashboard(props: DashboardProps) {
  const { sector, theme, imported, plans, basemap, isAdmin, busy, onBulk } = props
  ...
  const addPlan = (pick = false) => {
    openPlans()
    setPlanId(null)
    editor.open()
    if (pick) editor.startPicking()
  }
    ...
    <div className="rc-console">
      ...
      {panels.filtersOpen && (
        <FilterPanel
          ...
          assumptions={
            <AssumptionsField {...assumed} canSave={isAdmin} busy={busy} onChange={(patch) => setAssumptions((a) => ({ ...a, ...patch }))} onSaveRating={props.onPlanRating} />
          }
      ...
      <div className="rc-stage" ref={clearArea}>
        ...
        {showEmpty && (
          <EmptyState
            coverage={network.coverage}
            canAdd={isAdmin && plans.available}
            onAdd={() => addPlan()}
            onPick={() => addPlan(true)}
            onBulk={() => onBulk(true)}
      ...
      <SupportCard
        ...
        onAddPlan={() => {
          ...
          openPlans()
          setPlanId(null)
          editor.openFor({ no: support.no, load: String(support.loadA), ...(support.at && { at: toPosition(support.at) }) })
        }}
      ...
      {plansOpen && plans.available && (
        <BackupPlansPanel
          ...
          onRating={async (value) => {
            const written = await props.onPlanRating(value)
      ...
      <PhoneActions
        ...
        covered={Boolean(panels.filtersOpen || (supportOpen && support) || (plansOpen && plans.available && !picking))}
