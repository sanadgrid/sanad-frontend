...
export class StationMarker extends L.CircleMarker {
  ...
  /** A plain circle instead of a square — the base layer of every station, so it is not mistaken for the shape a ticked layer draws. */
  round = false
  labelColor = '#000'
  haloColor = '#fff'
  ...
  _updatePath() {
    ...
    ctx.beginPath()
    if (this.round) ctx.arc(x, y, r, 0, Math.PI * 2)
    else if (ctx.roundRect) ctx.roundRect(x - r, y - r, r * 2, r * 2, this.corner)
