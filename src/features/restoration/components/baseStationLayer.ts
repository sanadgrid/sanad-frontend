...
export interface BaseStationLayer {
  ...
  /** Once painted, so the circles never flash in the library's own blue. */
  addTo: (map: L.Map) => void
  /** Over the lines and areas of the imported layers, so a station can always be clicked. */
  toFront: () => void
  remove: () => void
}

// quiet: smaller and paler than the square of a ticked layer, which says "look here"; round, so it is never mistaken for one
...
/**
 * Every station of the sector as a small circle, on the canvas the imported
...
export function drawBaseStations(map: L.Map, renderer: L.Renderer, points: StationPoint[], actionsOf: StationActions): BaseStationLayer {
  ...
  for (const point of points) {
    const marker = new StationMarker([point.at.lat, point.at.lng], { ...SQUARE, renderer, pane: IMPORTED_PANE, bubblingMouseEvents: false }, point.no, BASE_LABEL_ZOOM)
    marker.round = true
    pointOf.set(marker, point)
